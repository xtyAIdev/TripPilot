...
- Planner 控制 Executor：Planner 输出 `tool` 字段和可选 `params`，Executor 按计划选择工具、POI 关键词和路线点
...
- 执行过程结构化：新增 `AgentStepEvent`，UI 可展示需求解析、RAG、MCP、预算、Reflection 等阶段
- 预算可行性：预算工具输出城市消费系数、住宿来源、人均/日均消费、预算口径和缺口判断
...
- Streamlit UI：支持侧边栏参数录入、聊天、待确认项、动态工作流、调试轨迹查看、Markdown 下载和封面图生成
...
Agent-TripPilot/
  ...
  trip_pilot/
    ...
    schemas.py         # Pydantic 数据结构，含 TripState 和 AgentStepEvent
...
Planner 输出必须包含 `tool` 字段。Executor 只执行计划中选择的工具，并尽量使用 Planner 给出的 `params` 控制 POI 关键词、路线点和预算关注点。

示例：

```json
[
  {"step": 1, "name": "获取当前时间", "tool": "time", "reason": "解析本周末", "params": {}},
  {"step": 2, "name": "检索旅行知识", "tool": "rag", "reason": "获取稳定知识", "params": {}},
  {"step": 3, "name": "查询 POI", "tool": "poi", "reason": "验证景点和美食", "params": {"poi_keywords": ["博物馆", "美食街"]}}
]
```
...
当前已让 Planner 可以输出 `params`，Executor 会读取 `poi_keywords`、`route_points` 和 `budget_focus`。路线距离仍需进一步结构化：后续应把 POI 查询结果解析成稳定经纬度，再调用高德距离或公交路线工具做排序。
